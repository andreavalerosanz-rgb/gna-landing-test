...
<template>
  <header class="relative w-full h-dvh bg-cover bg-top bg-no-repeat" style="background-image: url('/girona.jpg');">
    ...
    <nav
      :class="[isScrolled ? '' : 'pt-[5dvh]', 'fixed top-0 left-0 w-full z-50 transition-all duration-300 text-white', navClasses]">
      <div class="px-6 md:px-16 lg:px-24 flex items-center justify-between transition-all duration-300">
      ...
      <div v-show="isMenuOpen"
        ...
        <ul class="flex flex-col px-6 py-6 gap-6 text-lg font-medium">
          ...
          <li class="pt-2">
            <a href="#reservar" @click="closeMenu"
              class="inline-block px-8 py-2.5 rounded-full border border-white hover:bg-white hover:text-brand-pink transition-colors">
              Reservar
            </a>
          </li>
        </ul>
    ...
    <div class="relative z-10 flex h-full items-end justify-center px-4 pb-25 md:pb-30">
      <h1 class="text-6xl md:text-8xl lg:text-[110px] font-bold tracking-tight text-white drop-shadow-lg">
        GNA HOTEL SOLUTIONS
      </h1>
    </div>
